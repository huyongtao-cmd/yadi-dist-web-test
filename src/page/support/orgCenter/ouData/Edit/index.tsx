...
import {
  getBaseConfig, getFinancialConfig, getClassCodeConfig
} from './config';
import './index.less';
...
export default class Detail extends PureComponent<Props, State> {
  ...
  getDetailById = async (id) => {
    ...
    if (res && res.success) {
      ...
      this.setState({
        formDataBase, formDataFinancial, addressTableData, bankTableData, cardTableData, formDataClass
      })
    } else {
      ElNotification({
        type: 'error',
  ...
  boforeSave = async (type) => {
    ...
    params = { ...params, ...basic };
    // 校验财务信息
    const financial = this.formFinancialRef && await this.formFinancialRef.validateFields();
  ...
  async onSave(params, type) {
    ...
    let response: any;
    if (this.state.formDataBase.ouStatus === 'DRAFT' || this.state.formDataBase.ouStatus === '') {
  ...
  getButtons = ({ type, formDataBase }) => [
    ...
    },
    {
      ...
      type: 'primary',
      icon: <SubmitWhite />,
      disabled: type === 'view' || (formDataBase.ouStatus != '' && formDataBase.ouStatus != 'DRAFT'),
      location: 'left',
  ...
  render() {
    const { loading, formDataBase, formDataFinancial, type, formDataClass,
    ...
    return (
      <>
        <Spin spinning={loading}>
          ...
          <AddressTable status={formDataBase.ouStatus} type={type} tableData={addressTableData} onRef={(ref) => (this.addressTableRef = ref)} />
          <BankTable status={formDataBase.ouStatus} type={type} tableData={bankTableData} onRef={(ref) => (this.bankTableRef = ref)} />
          <LicensesTable status={formDataBase.ouStatus} type={type} tableData={cardTableData} onRef={(ref) => (this.cardTableRef = ref)} />
          <ElCard title='类别码'>
            <ElForm
