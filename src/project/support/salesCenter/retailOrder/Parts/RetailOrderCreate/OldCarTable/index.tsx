...
class OldCarTable extends PureComponent<any, any> {
  ...
  handleAdd = async () => {
    const formRefs = await this.props.baseFormRef.validateFields();
    await this.props.oldCarTableRef.quitEditState();
    this.props.oldCarTableRef.addRow({
  ...
  handleRemove = async ({ selectedRowKeys }) => {
    await this.props.oldCarTableRef.quitEditState();
    this.props.oldCarTableRef.removeRowsByKeys(selectedRowKeys, 'rowKey');
  }
  ...
  render() {
    return (
      <ElEditTable
        ...
        actionButtons={getTableActionButtons(this.handleAdd, this.handleRemove)}
