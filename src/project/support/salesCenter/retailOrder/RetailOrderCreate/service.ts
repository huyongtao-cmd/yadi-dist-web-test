...
// 根据车架号和门店id获取开票信息
export const getpaperInfoBySerialNo = (data: any) => {
  console.log(data);
  return requests('/yd-inv/Inv/ydSerial/checkInvSerial', {
    method: 'post',
    query: data
  });
};
...
// 身份证号校验
export const validateUserIdCard = (card: any) => {
  return requests(`/yd-sale/salSo/validateUserIdCard/${card}`, {
    method: 'get'
  });
};

// 通过门店id获取该门店下的发票信息
export const findInvoiceInfo = (id: any) => {
  return requests(`/yd-user/org/orgStore/getDetailsById/${id}`, {
    method: 'get'
  });
};
