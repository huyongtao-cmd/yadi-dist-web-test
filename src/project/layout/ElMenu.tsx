...
import { RightBlack, AppstoreBlack } from '@/components/el/ElIcon';
import {
  StarFilledYellow,
  StaroutlinedBlack,
  WrongBlack
...
@observer
class ElMenu extends React.Component<Props, State> {
  authMobx: any;
  ...
  collectMenu = (e, route) => {
    ...
    this.setState(
      {
        collectionList: [...collectionList, route]
      },
      () => {
        localStorage.setItem('el-collection', JSON.stringify(collectionList));
      }
    );
  };
  unCollectMenu = (e, route) => {
    ...
    this.setState(
      {
        collectionList: collectionList.filter((v) => v.name !== route.name)
      },
      () => {
        localStorage.setItem('el-collection', JSON.stringify(collectionList));
      }
            ...
            <Masonry>
              {Array.isArray(v.routes) &&
                v.routes.filter(this.authMenufilter).map((route) => {
                  if (route.name === 'inventoryserialno' || route.name === 'inventoryCheck' || route.name === 'administration' || route.name === 'numbering') {
                    return (
                      <div
                        key={route.name}
                  ...
                  } else if (route.name === 'saleReport' || route.name === 'dealer' ||
                    route.name === 'servingStation' || route.name === 'bikeDoc' ||
                    route.name === 'empList' || route.name === 'udc') {
                    return (
                      <List
                        key={route.name}
                        size='small'
                    ...
                    if (Array.isArray(route.routes) && route.routes.filter(this.authMenufilter).length !== 0) {
                      return (
                        <List
                          key={route.name}
                          size='small'
