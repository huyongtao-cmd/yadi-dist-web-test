 ...
 */
  ...
  handleValidRange = (data) => {
    return {
      ...
      erpTransferDateS:
        data.erpTime &&
        data.erpTime[0] &&
        dayjs(data.erpTime[0]).startOf('day').format('YYYY-MM-DD HH:mm:ss'),
      erpTransferDateE:
        data.erpTime &&
        data.erpTime[1] &&
        dayjs(data.erpTime[1]).endOf('day').format('YYYY-MM-DD HH:mm:ss'),
    }
  }
  // 导出
  handleExport = async (rowKeys) => {
    ...
    commonExport({
      url: '/yst-fin/fin/salSettle/export',
      params,
      fileName: '销售结算单'
    });
  }
  ...
  render() {
    return (<>
      <ElSearchTable
        id='SaleStatement'
        tableId='fin_sale_SaleStatementList'
        rowKey='id'
        bordered
