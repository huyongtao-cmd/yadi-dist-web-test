...
const getTableColumns = (type): Array<ElEditTableColumns> => {
  ...
  return [
      ...
      field: ({ formRef, record }) => {
        ...
        return {
          formOption: {
            ...
            props: {
              min: 0,
              // max: maths.sub(record.poQty || 0, record.shippedQty || 0),
              disabled: ['ALL'].includes(record.itemType?.udcVal) || ['ALL'].includes(itemName?.itemType),
              onChange: (e) => {
                if (e > maths.sub(record.poQty || 0, record.shippedQty || 0)) {
                  ElNotification({
                    type: 'warning',
    ...
    // },
    {
      ...
      cellRender: (text, record) => record && record.brandName
    },
    {
      title: '备注',
      width: 200,
      ...
      editable: true,
      field: () => {
        // const flag = formRef.formRef.getFieldValue('flag');
        return {
          formOption: {
            type: '$textArea',
            props: { autoSize: { minRows: 1, maxRows: 3 } }
      ...
      // selectMapping: async ({ changedValues, formRef, editTableRef }) => {
      //   console.log(changedValues, formRef, 'changedValueschangedValues;;;;;;')
      // },
    }
  ].filter(Boolean);
};
